    ...
    function checkLogin(login, error, submit) {
        if (login.val().trim() != '') {
            if(login.val().search(patternLogin) != 0){
                error.text('Only English letters and numbers (the first character must be a letter)');
   ...
   $('#signin').click(() => {
    ...
    $('#signupForm').submit((event) => {
        ...
        if ($login.length < 5 || $login.length > 20) {
            $("#errorSignup").text("Enter login (from 5 to 20 characters)");
            return false;
        } else if ($email == "") {
            $("#errorSignup").text("Enter email");
            return false;
        } else if ($password1.length < 6) {
            $("#errorSignup").text("Enter password \n" +
                "(at least 6 characters)");
            return false;
        } else if ($password2 == "") {
            $("#errorSignup").text("Repeat password");
            return false;
        } else if ($password1 != $password2) {
            $("#errorSignup").text("Passwords don't match");
            return false;
        }
        $("#errorSignup").text("");
    ...
    $('#signinForm').submit((event) => {
        ...
        if ($email == "") {
            $("#errorSignin").text("Enter email");
            return false;
        } else if ($password.length < 6) {
            $("#errorSignin").text("Enter password (at least 6 characters)");
            return false;
        }
        $("#errorSignin").text("");
